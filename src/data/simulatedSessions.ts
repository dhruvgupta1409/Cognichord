...
export function getSimulatedSessions(): SimSession[] {
  ...
  for (let i = 0; i < 5000; i++) {
    ...
    sessions.push({ instrument, durationMin, complexity, frequencyPerWeek: freq, mode, bpm, npi, bdnfDelta, dopamineIndex });
  }
...
export function aggregateByInstrument(sessions: SimSession[]) {
  ...
  for (const s of sessions) {
    const k = s.instrument;
    const e = map.get(k) ?? { npi: 0, bdnf: 0, da: 0, n: 0 };
    map.set(k, { npi: e.npi + s.npi, bdnf: e.bdnf + s.bdnfDelta, da: e.da + s.dopamineIndex, n: e.n + 1 });
  }
...
export function aggregateByDuration(sessions: SimSession[]) {
  ...
  for (const s of sessions) {
    ...
    for (const b of buckets) {
      const d = Math.abs(s.durationMin - b);
